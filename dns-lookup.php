...
if (isset($_POST['dns_checker'])) {
  ...
  if (preg_match($domain_regex, $domain_name)) {
    if (isset($dns_types[$get_dns_type])) {
    ...
    $message = '<span class="ip-address h4">' . htmlspecialchars($domain_name) . '</span> is not a valid domain name!';
  }
}
  ...
  <main>
        ...
        <div class="single-page all-page">
          <div class="row mb-4">
            <div class="col-lg-12 mb-3">
              <div class="h5 mb-0">DNS Records</div>
              <p>DNS records for <strong><?php echo htmlspecialchars($domain_name); ?></strong></p>
              <figure class="wp-block-table is-style-stripes">
                <table>
                  ...
                  <?php foreach ($dnsdata as $record) :
                    $value = $record['ip'] ?? $record['txt'] ?? $record['target'] ?? $record['mname'] ?? $record['ipv6'] ?? 'N/A';
                  ?>
                    <tr>
    ...
    <div class="container">
      <div class="row mb-3">
        ...
        <p class="mb-0">Use this tool to check an IP address against the most common blacklist databases. If you believe
          a blacklist or
          blocklist wrongfully added your IP address or mail server, this test helps you confirm and determine what to
          do
          next.</p>
      </div>
    </div>
    ...
    <div class="tools-bg-flex mb-5">
      <div class="container">
        <div class="row">
          ...
          <p>Domain name blacklists don’t notify users when their IP address goes on a blocklist. Users need to check
            name
            blacklists to find their status.
          </p>
          <p class="mb-0">Therefore, blacklist checks are essential for users, mail servers, or businesses who believe
            that they have a
            blacklisted IP address. If your IP is blocked, you'll show up on the list. Using the tool above is the
            easiest
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <p>Blocklists add IP addresses for many reasons. The intention is to protect other websites and users from
            ...
            normal, safe IP is blocked. Then, a user who hasn't done anything wrong will see their address in an IP
            block
            lookup.</p>
          <p>You may do a blacklist check online and find your own IP on a blacklist. A blacklist typically adds an IP
            for
